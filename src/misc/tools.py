...
def EnumerateSchematic(schematic: Schematic, parts_list: list) -> dict:
    ...
    Params:
        schematic (Schematic): Root schematic parsed by KiUtils
    ...
    def check_symbol(schematic: Schematic, symbol_uuid: str, parts_dict: dict, reference: str) -> bool:
        ...
        for symbol in schematic.schematicSymbols:
            if symbol.uuid in symbol_uuid:
                ...
                if not symbol.inBom:
                    logger.info(f'Symbol {symbol.libraryIdentifier} ({reference}) skipped as its not marked "in_bom"..')
                    return True
                ...
                for property in symbol.properties:
                    if property.key == user.IPN_FIELD_NAME:
                        # Check if the symbol's IPN is on the project's part list
                        if not property.value in parts_list:
                    ...
                    logger.warning(f'Skipping symbol {symbol.libraryIdentifier} ({reference}) as it has no IPN assigned ..')
                    return True
        return False
